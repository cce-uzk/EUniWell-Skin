...
| `release_10-euniwell` | ILIAS 10 |
| `release_11-euniwell` | ILIAS 11 |

**You are currently on `release_10-euniwell` (ILIAS 10).**
...
```bash
git fetch upstream
git switch -c release_12-euniwell --track upstream/release_12
git push -u origin release_12-euniwell
```

**Note:** The Delos SCSS structure may change between major versions and require manual adaptation before the skin compiles correctly.

* As of ILIAS 10, the delos SCSS files moved into a `delos/` subdirectory. This means import paths in `euniwell.scss` (e.g. `./010-settings/` → `./delos/010-settings/`) and the deep relative paths inside `skin_euniwell/010-settings/` (e.g. `../../030-tools/...` → `../../delos/030-tools/...`) must be reviewed and updated when setting up a new version branch.
...
git checkout release_10-euniwell
